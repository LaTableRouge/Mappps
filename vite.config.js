...
if (chore !== 'ci') {
  if (isProduction) {
    stringReplaceOpenAndWrite(
      ...
      [
        {
          from: /\bdefine\([ ]?'MPS_VITE_DEVELOPMENT',[ ]?true[ ]?\);/g,
          to: "define('MPS_VITE_DEVELOPMENT', false);"
        }
      ]
    )
  } else {
    stringReplaceOpenAndWrite(
      ...
      [
        {
          from: /\bdefine\([ ]?'MPS_VITE_DEVELOPMENT',[ ]?false[ ]?\);/g,
          to: "define('MPS_VITE_DEVELOPMENT', true);"
        }
      ]
    )
  }
      ...
      ? run({
        ...
        input: [
          ...
          chore === 'all' || chore === 'prettier:js'
            ? {
              name: 'prettier:js',
              run: [`${beautifyObject.js_prettier.config} ${beautifyObject.js_prettier.files.length > 1 ? `{${beautifyObject.js_prettier.files.join(',')}}` : beautifyObject.js_prettier.files.join(',')}/**/*.{js,jsx}`],
            }
            : false,
