...
export default function Wizard({ attributes, postTypes, setAttributes, setQueriedPosts }) {
  ...
  return (
    <>
      {postTypes.length
        ? (
        <Placeholder className="maaaps__wizard" instructions={__('Select the data source of the items to be displayed on the map.', 'maaaps')} label={__('Data source', 'maaaps')}>
